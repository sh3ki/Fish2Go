...
export default function AdminSales() {
  ...
  const fetchOrders = (date, page) => {
    axios.get(`/api/orders?date=${date}&page=${page}`)
      .then((response) => {
            ...
            <tbody>
              {filteredOrders.length > 0 ? (
                  ...
                  <React.Fragment key={index}>
                    {order.items.map((item, itemIndex) => (
                      <tr key={itemIndex} className="text-center">
                        {itemIndex === 0 && (
                          <td className="border px-4 py-2" rowSpan={order.items.length}>{order.id}</td>
                        )}
                        ...
                        {itemIndex === 0 && (
                          <>
                            <td className="border px-4 py-2" rowSpan={order.items.length}>{order.subtotal}</td>
                            <td className="border px-4 py-2" rowSpan={order.items.length}>{order.tax}</td>
                            <td className="border px-4 py-2" rowSpan={order.items.length}>{order.discount}</td>
                            <td className="border px-4 py-2" rowSpan={order.items.length}>{order.total}</td>
                            <td className="border px-4 py-2" rowSpan={order.items.length}>{order.payment}</td>
                            <td className="border px-4 py-2" rowSpan={order.items.length}>{order.change}</td>
                            <td className="border px-4 py-2" rowSpan={order.items.length}>{order.status}</td>
                            <td className="border px-4 py-2" rowSpan={order.items.length}>
                ...
                <tr className="text-white border-b border-gray-600">
                  <td className="p-2" colSpan={12}>No data available.</td>
                </tr>
              )}
            </tbody>
          </table>
